...
function StatusBadge({ status }: { status: number }) {
  if (status === 0) return (
    <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-semibold text-emerald-700 ring-1 ring-emerald-200/50 dark:bg-emerald-900/30 dark:text-emerald-200 dark:ring-emerald-800/60">
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-400 dark:text-gray-500">Quản lý tài khoản hệ thống</p>
        </div>
        {canManageUsers && (
          <button
            type="button"
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex flex-1 gap-2 sm:max-w-sm">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-xl border border-gray-200 bg-white py-2.5 pl-9 pr-10 text-sm focus:border-indigo-400 focus:outline-none focus:ring-4 focus:ring-indigo-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 dark:focus:border-indigo-500 dark:focus:ring-indigo-500/30"
            />
            {searchInput.trim().length > 0 && (
              <button
                type="button"
                onClick={() => setSearchInput("")}
                className="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-gray-700 dark:hover:text-gray-200 transition-colors"
        ...
        <select
          ...
          className="rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-700 focus:border-indigo-400 focus:outline-none focus:ring-4 focus:ring-indigo-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 dark:focus:border-indigo-500 dark:focus:ring-indigo-500/30"
        >
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
              {isLoading ? (
                ...
                data?.content.map((user, i) => (
                  <tr
                    key={user.id}
                    className="group cursor-pointer transition-colors hover:bg-indigo-50/40 dark:hover:bg-indigo-900/20"
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2.5">
                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-xs font-bold text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-200">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-sm text-gray-500 dark:text-gray-400">
        <span>Tổng <strong className="text-gray-800 dark:text-gray-100">{data?.amount ?? 0}</strong> người dùng</span>
        {totalPages > 1 && (
...
function ActionBtn({ title, color, icon, onClick }: { title: string; color: string; icon: React.ReactNode; onClick: (e: React.MouseEvent) => void }) {
  const colors: Record<string, string> = {
    blue: "text-indigo-500 hover:bg-indigo-50 dark:text-indigo-300 dark:hover:bg-indigo-900/30",
    amber: "text-amber-500 hover:bg-amber-50 dark:text-amber-300 dark:hover:bg-amber-900/30",
...
function PageBtn({ disabled, onClick, children }: { disabled: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className="rounded-xl border border-gray-200 px-3 py-1.5 text-sm font-medium text-gray-600 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800"
    >
      {children}
    </button>
  );

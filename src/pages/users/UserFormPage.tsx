        ...
        <ModalBody>
          {readOnly && loadPending ? (
            <div className="space-y-3">
              {[1, 2, 3, 4, 5].map((i) => <div key={i} className="h-10 animate-pulse rounded-xl bg-gray-100 dark:bg-gray-700" />)}
            </div>
          ) : (
            <FormFields
              form={form}
  ...
  return (
    <div className="mx-auto max-w-lg">
      <div className="mb-6 flex items-center gap-3">
        <button type="button" onClick={() => navigate({ to: "/users" })} className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 transition-colors dark:text-gray-400 dark:hover:text-gray-200">
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
        ...
        <div className="mt-6 flex gap-3">
          <button type="button" onClick={() => navigate({ to: "/users" })} className="flex-1 rounded-xl border border-gray-200 py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700">Hủy</button>
          {!effectiveReadOnly && (
